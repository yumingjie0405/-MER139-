...
def main():
    ...
    cam.stream_on()
    cv2.namedWindow("--- 标定 ---", cv2.WINDOW_AUTOSIZE)
    ...
    while True:
        ...
        if len(cam.data_stream) > 0:
            ...
            #draw_1 = cv2.rectangle(numpy_image.copy(),(320,256),(960,768),(0,255,50),2)  #640*512
            draw_1 = cv2.rectangle(numpy_image.copy(),(320,272),(960,752),(0,255,0),2)
            #draw_1 = cv2.rectangle(numpy_image.copy(), (320, 112), (960, 912), (0, 255, 0), 2) #600*800
            #draw_1 = cv2.rectangle(numpy_image.copy(),(240,212),(1040,812),(0,255,0),2)
            # show acquired image
            # r = cv2.findChessboardCorners(numpy_image,(12,8))

            cv2.imshow("--- 标定 ---", draw_1)
            # print height, width, and frame ID of the acquisition image
            key = cv2.waitKey(1) & 0xFF
            if (key == ord('s')):
                print("Frame ID: %d   Height: %d   Width: %d"
                      % (raw_image.get_frame_id(), raw_image.get_height(), raw_image.get_width()))
                #corped = numpy_image[112:912,320:920]  #[y0,y1][x0,x1]
                #corped = numpy_image[256:768,320:960]   #640*512
                corped = numpy_image[272:752,320:960]   #640*480
                #corped = numpy_image[212:812,240:1040]
                filename = time.strftime("%H_%M_%S", time.localtime())
                #cv2.imwrite(filename+".bmp",numpy_image)q
