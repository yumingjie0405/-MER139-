...
def main():
    ...
    while True:
        ...
        if len(cam.data_stream) > 0:
            ...
            draw_1 = cv2.rectangle(numpy_image.copy(),(320,256),(960,768),(0,255,50),2)
            ...
            while(1):
                ...
                out = cv2.VideoWriter('output.mp4', fourcc, 20, (640, 512))
                cv2.imshow("corped", corped)
                cv2.waitKey(1)
                out.write(corped)
                #cv2.imshow('video', corped)
                if(key == ord('q')):
